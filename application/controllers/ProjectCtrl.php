...
class ProjectCtrl extends CI_Controller
{
    ...
    public function project_view($id = false)
    {
        ...
        $data['ngo_list'] = $this->db->get_where('user_table', array('role_id' => '2', 'status' => 'Active'))->result();
    ...
    public function project_insert_update($id = false)
    {
        ...
        if ($id == "") {
            ...
            $data['project_insert_update']['start_date'] = $now->format('d F, Y');
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($id == "") {
                ...
                if ($insert_id) {
                    ...
                    redirect(base_url('projectentry'), 'refresh');
                }
            } else {
    ...
    public function proj_doc_upload()
    {
        ...
        if ($this->input->post('action')) {
            if (!empty($_FILES['images']['name'])) {
                $pathToUpload = 'upload/Project/' . $proj_id . '/Image';
            ...
            if (!empty($_FILES['document']['name'])) {
                $pathToUpload = 'upload/Project/' . $proj_id . '/Documents';
                ...
                for ($i = 0; $i < $filesCount; $i++) {
                    ...
                    $config1['allowed_types'] = 'pdf|docx|doc|jpg|png|jpeg';
                ...
                if (!empty($uploadData1)) {
                    ...
                    $statusMsg = $insert ? 'Files uploaded successfully.' : 'Some problem occurred, please try again.';
                    $this->session->set_flashdata('msg', "1");
                    redirect(base_url('projectentry'));
                }
            }
